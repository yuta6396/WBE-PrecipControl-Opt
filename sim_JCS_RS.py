...
random_iter_vec =  [15, 15, 20, 50, 50, 50, 50, 50]          #{10, 20, 20, 50]=10, 30, 50, 100と同値
# random_iter_vec = [3, 2]  
...
def random_search(objective_function, bounds, f_in, f_RS, RS_ratio_matrix, RS_time_matrix, trial_i, optimization_results):
    # 各試行の結果を格納するデータ構造
    trial_data = {
    ...
    for i in range(len(random_iter_vec)):
        ...
        RS_ratio_matrix[i, trial_i] = objective_value
        RS_time_matrix[i, trial_i] = elapsed_time
        
        # 実験結果をJSON形式で格納
        experiment_data = {
            "experiment_index": i,
            "num_evaluations": int(cnt_vec[i]),
            "optimization_result": {
                "min_value": float(best_score),
                "min_input": [float(x) for x in best_params],
                "elapsed_time": float(elapsed_time),
                "input_history": [[float(x) for x in candidate] for candidate in input_history]
            },
            "simulation_result": {
...
def main():
    ...
    RS_time_matrix = np.zeros((len(random_iter_vec), trial_num))
    ...
    with open(RS_file, 'w') as f_RS, open(RS_input_file, 'w') as f_in:
        for trial_i in range(trial_num):
            ...
            random_search(black_box_function, bounds, f_in, f_RS, RS_ratio_matrix, RS_time_matrix, trial_i, optimization_results)

    # 結果行列をJSON形式で保存
    optimization_results["summary"]["RS_ratio_matrix"] = RS_ratio_matrix.tolist()
    optimization_results["summary"]["RS_time_matrix"] = RS_time_matrix.tolist()
